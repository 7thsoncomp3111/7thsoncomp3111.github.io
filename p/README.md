# Some notes on reveal.js

  - Press ```b``` to pause and blackout the presentation
  - Press ```s``` to show speaker notes, timer and next slide preview
  - Add `<aside>` within ```<section>``` to add speaker notes / script
